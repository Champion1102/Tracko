import Link from "next/link";
import { logoutAction } from "@/app/actions";
import { cookies } from "next/headers";
import { loadState } from "@/lib/state";
import { THEME_COOKIE, parseTheme } from "@/lib/theme";
import { HabitIcon } from "@/components/HabitIcon";
import { HerSettings } from "@/components/HerSettings";
import { PromiseCard } from "@/components/PromiseCard";
import { PushSetup } from "@/components/PushSetup";
import { SWRegister } from "@/components/SWRegister";

export const dynamic = "force-dynamic";

export default async function SettingsPage() {
  const s = await loadState();
  const theme = parseTheme((await cookies()).get(THEME_COOKIE)?.value);

  return (
    <div className="space-y-5">
      <SWRegister />

      <header className="px-1 pt-1">
        <h1 className="text-[26px] leading-none font-black text-text">
          {s.config.heroName ? `Hey ${s.config.heroName}` : "You"}
        </h1>
        <p className="mt-1.5 text-[13px] font-bold text-muted">
          Day {s.totals.daysElapsed} of {s.config.totalDays} · {s.totals.daysDone} days done
        </p>
      </header>

      <PromiseCard
        text={s.config.promiseText}
        signature={s.config.promiseSignature}
        signedAt={s.config.promiseAcceptedAt}
        name={s.config.heroName}
        daysElapsed={s.totals.daysElapsed}
      />

      <PushSetup vapidKey={process.env.NEXT_PUBLIC_VAPID_PUBLIC_KEY ?? ""} />
      <HerSettings config={s.config} theme={theme} />

      <section className="card p-4">
        <h2 className="text-[12px] font-bold text-faint">Your habits</h2>
        <ul className="mt-3 space-y-2.5">
          {s.habits.map((h) => (
            <li key={h.id} className="flex items-center gap-3">
              <span className="grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-surface-2 text-muted">
                <HabitIcon icon={h.icon} emoji={h.emoji} size={17} />
              </span>
              <span className="min-w-0 flex-1 truncate text-[13.5px] font-bold text-text">
                {h.name}
              </span>
            </li>
          ))}
        </ul>
        <p className="mt-3 text-[11.5px] leading-snug font-semibold text-faint">
          Want one changed, added or dropped? Ask{" "}
          {s.config.sponsorName || "whoever set this up"} — they hold the list.
        </p>
      </section>

      <Link href="/set-pin" className="press card flex items-center justify-between p-4">
        <span>
          <span className="block text-[13.5px] font-black text-text">Change your PIN</span>
          <span className="block text-[12px] font-semibold text-muted">
            The six digits you sign in with.
          </span>
        </span>
        <span className="text-[16px] text-faint">›</span>
      </Link>

      <form action={logoutAction}>
        <button className="w-full py-3 text-[12px] font-bold text-faint">Sign out</button>
      </form>
    </div>
  );
}
